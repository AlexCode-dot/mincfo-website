...
export default function CanvasBeam() {
  ...
  useEffect(() => {
    ...
    const drawGlow = (beamRadius: number, glowStrength: number) => {
      ...
      coreGradient.addColorStop(0, `rgba(103, 120, 222, ${0.08 * glowStrength})`);
      coreGradient.addColorStop(0.5, `rgba(103, 120, 222, ${0.19 * glowStrength})`);
      coreGradient.addColorStop(1, `rgba(103, 120, 222, ${0.06 * glowStrength})`);

      const haloGradient = ctx.createLinearGradient(0, 0, 0, height);
      haloGradient.addColorStop(0, `rgba(88, 104, 194, ${0.05 * glowStrength})`);
      haloGradient.addColorStop(0.5, `rgba(88, 104, 194, ${0.12 * glowStrength})`);
      haloGradient.addColorStop(1, `rgba(88, 104, 194, ${0.04 * glowStrength})`);
    ...
    const drawBeamDots = (t: number) => {
      ...
      for (let x = startX; x <= endX; x += dotSpacing) {
        ...
        for (let y = 0; y <= height; y += dotSpacing) {
          ...
          ctx.fillStyle = `rgba(153, 170, 233, ${intensity * 0.52})`;
          ctx.beginPath();
    ...
    const drawRain = () => {
      const { width, height } = sizeRef.current;
      ctx.save();
      ctx.strokeStyle = "rgba(110, 130, 214, 0.08)";
      ctx.lineWidth = 1;
